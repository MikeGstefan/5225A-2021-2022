...
Position distance_reset_left(int cycles){
    ...
    int left_get = l_reset_dist.get();
    int right_get = r_reset_dist.get();
		side_dist = l_dist.get();
		misc.print("Left:%d Right: %d, Side: %d", left_get, right_get, side_dist);
    if(!left_get) error_count++; else left.push_back(left_get);
    if(!right_get) error_count++; else right.push_back(right_get);
