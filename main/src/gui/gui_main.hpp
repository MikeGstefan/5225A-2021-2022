...
void GUI::setup(){ //Call once at start in initialize()
  ...
  go_to_xya.set_func([&x=xVal.val, &y=yVal.val, &a=aVal.val](){
    ...
    if (GUI::go("GO TO" + coord, "Press to move to target selected by sliders" + coord, 1000)) printf("sorry nathan\n");//move_to_target_sync(x, y, a, false);
  });
  go_home.set_func([](){if (GUI::go("GO TO (0, 0, 0)", "Press to go to starting point (0, 0, 0)", 1000)) printf("sorry nathan\n");});//move_to_target_sync(0, 0, 0, false);});
  go_centre.set_func([](){if (GUI::go("GO TO (72, 72, 0)", "Press to go to centre field (72, 72, 0)", 1000)) printf("sorry nathan\n");});//move_to_target_sync(72, 72, 0, false);});
