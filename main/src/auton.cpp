...
void high_short(){
  ...
  move_start(move_types::tank_rush, tank_rush_params({104.0, 72.0, 0.0}, false, 127.0, 1.0,false));
...
void high_match_loads(){
   move_start(move_types::tank_point, tank_point_params({117.0, 42.0,0.0}));
  // return;
...
void high_wp_goal(){
  ...
  move_start(move_types::tank_point, tank_point_params({104.0, 35.0,0.0}, false, 127.0, 1.0, true, 6.4, 70.0, 0.0, 0, {0.75, 0.75}, min_move_power_y +15));
  // drivebase.set_state(0);
  // Task([](){detect_interference();});
  // delay(500);
  f_lift.set_state(f_lift_states::move_to_target, 1);
  move_wait_for_complete();
  move_start(move_types::turn_angle, turn_angle_params(-90.0, true, true, 5.0,0.0,10.0,20.0,127.0,2000,45,30));
  delay(50);
  move_start(move_types::turn_angle, turn_angle_params(-90.0, true, true, 5.0,0.0,10.0,20.0,127.0,2000,45,30));
  move_start(move_types::tank_point, tank_point_params({130.0, 35.0,0.0}), false);
  while(tracking.x_coord < 112.0)delay(10);
  b_detect_goal();
  b_claw_obj.set_state(b_claw_states::tilted);
  misc.print(" goal pos: %.2f, %.2f, %.2f\n", tracking.x_coord, tracking.y_coord, rad_to_deg(tracking.global_angle));
  Task([](){
    delay(150);
...
void high_awp(){ 
  ...
  flatten_against_wall();
  delay(100); //
  tracking.reset(DIST_FRONT,  reset_dist_l.get_dist(), -90.0);
  b_lift.set_state(b_lift_states::move_to_target,b_top);

   move_start(move_types::tank_point, tank_point_params({15.0, tracking.y_coord, -90.0}));
  move_start(move_types::turn_angle, turn_angle_params(angle_to_point(20.0, 15.0) +180));
  hitch.set_state(0);
  move_start(move_types::tank_point, tank_point_params({20.0, 15.0, -90.0}, false));
  move_start(move_types::turn_angle, turn_angle_params(angle_to_point(40.0, 12.0) +180));
  move_start(move_types::tank_point, tank_point_params({40.0, 12.0, -90.0}, false, 60.0), false);
   detect_hitch(45, 1);
   hitch_obj.set_state(hitch_states::grabbed);
  move_stop();
  drivebase.brake();
...
void high_line(){
  move_start(move_types::tank_point, tank_point_params({119.0, 37.0,0.0}, false, 127.0, 1.0, true, 6.4, 70.0,0.0, 0,{4.0, 0.5}));
  
  move_start(move_types::turn_angle, turn_angle_params(0.0));
  b_lift.motor.move(-127);
  
  move_start(move_types::tank_point, tank_point_params({117.0, 70.0,0.0}, false,60, 1.0, true, 6.4, 70.0,0.0, 1900));
}

void high_plus(){ 
  move_start(move_types::turn_angle, turn_angle_params(-90.0));
  // b_lift.Subsystem::set_state(b_lift_states::intake_on);
  move_start(move_types::tank_point, tank_point_params({66.0, 44.0,0.0}, false,70));
  move_start(move_types::tank_point, tank_point_params({60.0, 41.0,0.0}, false, 70, 1.0, true,6.4,70.0,0.0,0,{1.0,1.0}));
  // delay(100);s
  tilt_goal();
  b_claw_obj.set_state(b_claw_states::flat);
}
...
void high_tall(){ 
  ...
  // f_claw_obj.set_state(f_claw_states::grabbed);
  // hitch_obj.set_state(hitch_states::grabbed);
  move_start(move_types::tank_rush, tank_rush_params({72.0, 70.5, 0.0}, false, 127.0, 1.0,false));
  f_claw_obj.set_state(f_claw_states::grabbed);
  // drivebase.brake();
  drivebase.move(0.0,0.0);
  delay(150);
  // drivebase.brake();
  // delay(300);
  move_start(move_types::turn_angle, turn_angle_params(-225.0, true, false, 5.0, 0.0, 10.0, 20.0, 127.0, 3000, min_move_power_a + 20));
  high_wp_goal();
...
void low_short(){
  ...
  drivebase.brake();
  delay(300);
  move_start(move_types::tank_point, tank_point_params({24.0, 13.0, 90.0}));
  f_lift.set_state(f_lift_states::move_to_target, 1);
  move_start(move_types::turn_angle, turn_angle_params(angle_to_point(40.0, 12.0) +180));
...
void low_center(){
  move_start(move_types::tank_point, tank_point_params({70.5, 70.5, 45.0}, false, 60.0, 1.0, true, 6.4, 70.0), false);
  b_lift.Subsystem::set_state(b_lift_states::intake_on);
  while(tracking.y_coord < 55.0)delay(10);
  f_detect_goal();
  move_stop();
  drivebase.move(0.0,0.0);
  delay(300);
  ...
  move_start(move_types::tank_point, tank_point_params({36.0, 36.0, 90.0}));
  f_lift.set_state(f_lift_states::move_to_target, 1);
  // f_claw_bj.Subsystem::set_state(f_)
   f_claw_obj.set_state(f_claw_states::grabbed);
  return;
  move_start(move_types::turn_angle, turn_angle_params(-90.0));
  move_start(move_types::tank_point, tank_point_params({20.0, 13.0, 90.0}));
  move_start(move_types::turn_angle, turn_angle_params(angle_to_point(40.0, 12.0) +180));
  move_start(move_types::tank_point, tank_point_params({40.0, 12.0, -90.0}, false, 40.0), false);
  //  detect_hitch(45, 1);
  b_detect_goal();
  ...
  drivebase.brake();
  b_lift.Subsystem::set_state(b_lift_states::intake_on);
  move_start(move_types::tank_point, tank_point_params({20.0, 12.0, -90.0}, false, 70.0));
