...
void skills_park(){
  ...
  f_lift.set_state(f_lift_states::move_to_target, 4); //2170
  b_lift.set_state(b_lift_states::move_to_target, b_lift.prog_positions.size()-1); //2800
  while(f_lift.get_target_state() != f_lift_states::idle)delay(10);
  flatten_against_wall();
  delay(100);
  ...
  move_start(move_types::turn_angle, turn_angle_params(90.0));
  move_start(move_types::tank_point, tank_point_params({32.0, 11.75, 90.0}));
  ...
  b_lift.set_state(b_lift_states::move_to_target, 5); //2800
  f_lift.set_state(f_lift_states::move_to_target, 1); //1280
