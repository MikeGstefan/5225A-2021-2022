...
void skills3(){ 
  ...
  move_start(move_types::tank_point, tank_point_params({79.0, 103.0, -45.0}), false);
  b_lift.set_state(b_lift_states::move_to_target, b_backup);
  Task([]{tilt_goal();});
  move_wait_for_complete();
  ...
  f_lift.set_state(f_lift_states::move_to_target, f_backup);
  int lift_timer = millis();
  while(f_lift.get_target_state() != f_lift_states::idle && (lift_timer - millis()) < 500)delay(10);
  f_claw(0);
  delay(100);
  move_start(move_types::tank_point, tank_point_params(polar_to_vector_point(tracking.x_coord, tracking.y_coord, -3.0, rad_to_deg(tracking.global_angle), rad_to_deg(tracking.global_angle)),  false, 127.0,1.0,true, 40.0, 70.0, 0.0, 0, {0.5,0.5}, 30.0));
  move_start(move_types::turn_angle, turn_angle_params(140.0, true, false));
  move_start(move_types::tank_point, tank_point_params(polar_to_vector_point(tracking.x_coord, tracking.y_coord, -8.0, rad_to_deg(tracking.global_angle), rad_to_deg(tracking.global_angle)),  false, 127.0,1.0,true, 20.0, 70.0, 0.0));
