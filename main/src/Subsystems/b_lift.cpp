...
void B_Lift::handle(bool driver_array){
  ...
  switch(get_state()){
    ...
    case b_lift_states::move_to_target: //moving to target
      ...
      { //don't pwr the motor if the state is no longer move to target
        ...
        if(abs(output) < 50) output = 50 * sgn(output); //enforces a minimum of 30 pwr
        motor.move(output);
      }
      ...
      if(bad_count > 10 && state != b_lift_states::manual){
        ...
        printf2("LIFT SAFETY TRIGGERED %f, %f", target, b_lift_pot.get_value());
    ...
    case b_lift_states::manual:
      lift_pwr = master.get_analog(ANALOG_RIGHT_X);
      //if master controller joystick isn't active, take partner input instead
      if(abs(lift_pwr) < 10) lift_pwr = partner.get_analog(ANALOG_RIGHT_Y);
      //holds motor if joystick is within deadzone or lift is out of range
      if (abs(lift_pwr) < 10 || (lift_pwr < 0 && b_lift_pot.get_value() <= driver_positions[0]) || (lift_pwr > 0 && b_lift_pot.get_value() >= driver_positions[driver_positions.size() - 1])){
        lift_pwr = 0;
      }
      //if back lift is below a threshold and the hitch is open, close it to not break it
      if(b_lift_pot.get_value() < 1600 && lift_pwr < 0){
        if(hitch_obj.get_state() != hitch_states::grabbed){
          hitch_obj.set_state(hitch_states::grabbed);
        }
        //if the goal is detected, don't pwr the lift
        if(hitch_dist.get() < 25) lift_pwr = 0;
      }
...
void B_Lift::shift(bool piston_state){
  ...
  state_log.print("after_switch_state:%s", b_lift.state_names[static_cast<int>(after_shift_state)]);
...
void B_Lift::elastic_util(int high){ //1011 as of April 10th
  ...
  while(fabs(b_lift_m.get_position() - high) > 15) b_lift_m.move(127);
  move_timer.print();
  elastic_b_up_time = move_timer.get_time();
  master.print(1, 0, "up time: %d", elastic_b_up_time);

  move_timer.reset();
  while(fabs(b_lift_m.get_position() - 0) > 15) b_lift_m.move(-127);
  move_timer.print();
